#!/usr/bin/env bash
# DigitalOcean Ubuntu - Sunucu: 64.226.98.17
set -euo pipefail

# ---------------------------------------- Bilgiler ----------------------------------------
SERVER_IP="64.226.98.17"
GITHUB_HESAP="mpiriyilmaz"
GITHUB_REPO="aras_test_v15"
DB_NAME="arasomtest"
DB_USER="postgres"
...
if [ ! -d "/opt/aras_test_v15/.git" ]; then
  echo "[INFO] SSH ile klon deneniyor..."
  if git clone git@github.com:mpiriyilmaz/aras_test_v15.git; then
    :
  else
    echo "[WARN] SSH klon başarısız. HTTPS fallback denenecek."
    if [ -n "${GITHUB_PAT:-}" ]; then
      git clone https://mpiriyilmaz:${GITHUB_PAT}@github.com/mpiriyilmaz/aras_test_v15.git
    else
  ...
  git -C "/opt/aras_test_v15" pull --rebase || true
fi

git config --global --add safe.directory /opt/aras_test_v15 || true

# manage.py repo kökünde
ls -la "/opt/aras_test_v15/manage.py" || true

# ---------------------------------------- venv & paketler ----------------------------------------
cd /opt/aras_test_v15
python3 -m venv .venv
...
ALLOWED_HOSTS=64.226.98.17,localhost,127.0.0.1
CSRF_TRUSTED_ORIGINS=http://64.226.98.17,https://64.226.98.17,http://64.226.98.17:8000
...
[Unit]
Description=Gunicorn for aras_test_v15
...
User=root
Group=www-data
WorkingDirectory=/opt/aras_test_v15
Environment="PATH=/opt/aras_test_v15/.venv/bin"
Environment="DJANGO_SETTINGS_MODULE=core.settings"
ExecStart=/opt/aras_test_v15/.venv/bin/gunicorn --workers 3 --timeout 120 --umask 007 --bind unix:/run/gunicorn_v1/gunicorn.sock core.wsgi:application
...
sudo tee /etc/nginx/sites-available/aras_test_v15.conf > /dev/null <<'NGINX'
server {
    listen 80;
    listen [::]:80;
    server_name _;
    ...
    # STATIC_ROOT: /opt/aras_test_v15/staticfiles/
    location /static/ {
        alias /opt/aras_test_v15/staticfiles/;
        access_log off;
        expires 30d;
    }
...
# Etkinleştir
sudo ln -sf /etc/nginx/sites-available/aras_test_v15.conf /etc/nginx/sites-enabled/aras_test_v15.conf
